...
| fileType        | 手动指定文件类型：'pdf'、'ofd'、'word'、'excel'                      | string? | --  | 自动检测   |
...
| useOfdPreview   | 使用带有tools组件的ofd预览组件（useOfdViewer都为false，默认使用liteofd手动渲染） | boolean | --  | true   |
| useOfdViewer    | 使用预览多页面ofd预览组件  （useOfdPreview都为false，默认使用liteofd手动渲染）   | boolean | --  | false  |
