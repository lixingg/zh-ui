...
interface BaseMapConfig {
  id: string; name: string; type: 'url' | 'ion' | 'arcgis' | 'tianditu' | 'custom'
  options: Record<string, any>; visible?: boolean
}
interface MarkerConfig {
  id: string; position: { lng: number; lat: number; height?: number }
  icon?: string; size?: number; popup?: { title?: string; content?: string }
  onClick?: (entity: Cesium.Entity) => void
}
interface PolylineConfig {
  id: string; positions: { lng: number; lat: number; height?: number }[]
  width?: number; color?: string; clampToGround?: boolean
}
interface PolygonConfig {
  id: string; positions: { lng: number; lat: number; height?: number }[]
  color?: string; outline?: boolean; outlineColor?: string; outlineWidth?: number
}
interface HeatmapConfig {
  data: { lng: number; lat: number; value: number }[]
  radius?: number; maxValue?: number; opacity?: number
}
interface VehicleTrackConfig {
  id: string; points: { lng: number; lat: number; time: number; height?: number }[]
  modelUrl?: string; speed?: number; clampToGround?: boolean; autoView?: boolean
  viewOffset?: { heading: number; pitch: number; range: number }
  pathColor?: string; pathWidth?: number
}
interface FlightTrackConfig {
  id: string; from: { lng: number; lat: number; alt: number }
  to: { lng: number; lat: number; alt: number }
  height?: number; arcFactor?: number; speed?: number; modelUrl?: string
  pathColor?: string; pathWidth?: number; autoView?: boolean
}
interface MapConfig {
  animation?: boolean; timeline?: boolean; baseLayerPicker?: boolean
  fullscreenButton?: boolean; homeButton?: boolean; sceneModePicker?: boolean
  infoBox?: boolean; selectionIndicator?: boolean; geocoder?: boolean
  navigationInstructionsInitiallyVisible?: boolean; navigationHelpButton?: boolean
  [key: string]: any
}
...
const props = defineProps<{
  ...
  vehicleTracks?: VehicleTrackConfig[]
  flightTracks?: FlightTrackConfig[]
}>()

const emit = defineEmits<{
  (e: 'ready', viewer: Cesium.Viewer): void
  (e: 'mapClick', event: { position: Cesium.Cartesian2; picked: any }): void
  (e: 'markerClick', entity: Cesium.Entity): void
}>()
...
function updateMarkers(markers: MarkerConfig[]) {
  ...
  markers.forEach(m => {
    ...
    const entity = new Cesium.Entity({
      id: m.id, position,
      billboard: {
        image: m.icon || defaultMarkerIcon,
        scale: m.size ? m.size / 32 : 1,
  ...
  lines.forEach(l => {
    lineDataSource.entities.add({
      ...
      polyline: {
        ...
        width: l.width || 2,
        material: Cesium.Color.fromCssColorString(l.color || '#FF0000'),
        clampToGround: l.clampToGround ?? false
      }
    })
  })
}
  ...
  polygons.forEach(p => {
    polyDataSource.entities.add({
      ...
      polygon: {
        ...
        outlineColor: Cesium.Color.fromCssColorString(p.outlineColor || '#FFFFFF'),
        outlineWidth: p.outlineWidth ?? 1
      }
    })
  })
}
...
function createHeatmapCanvas(config: HeatmapConfig, bounds: { west: number; east: number; south: number; north: number }): string {
  ...
  const maxValue = config.maxValue || Math.max(...data.map(p => p.value), 1)
  data.forEach(point => {
    ...
    const alpha = Math.min(point.value / maxValue, 1) * 0.9
...
function updateHeatmap(config: HeatmapConfig | null | undefined) {
  ...
  const extent = getExtent(config.data)
  const minSpan = 1.5
  const lngSpan = extent.east - extent.west
  const latSpan = extent.north - extent.south
  const bufferLng = Math.max((minSpan - lngSpan) / 2, 0.1)
  const bufferLat = Math.max((minSpan - latSpan) / 2, 0.1)
...
function updateVehicleTracks(tracks: VehicleTrackConfig[]) {
  const v = getViewer()
  vehicleTrackEntities.forEach(({ pathEntity, iconEntity }, id) => {
    if (!tracks.find(t => t.id === id)) { v.entities.remove(pathEntity); v.entities.remove(iconEntity); vehicleTrackEntities.delete(id) }
  })
  tracks.forEach(track => {
    ...
    const iconPositionProp = new Cesium.SampledPositionProperty()
    const start = Cesium.JulianDate.fromDate(new Date(correctedPoints[0].time))
    const stop = Cesium.JulianDate.fromDate(new Date(correctedPoints[correctedPoints.length - 1].time))
    ...
    const iconEntity = v.entities.add({
      ...
      orientation: useModel ? new Cesium.VelocityOrientationProperty(iconPositionProp) : undefined,
      billboard: useModel ? undefined : {
        image: defaultCarIcon,
        width: 80,
        height: 40,
...
function startVehicleTrack(id: string) {
  const v = getViewer()
  const pair = vehicleTrackEntities.get(id)
  if (!pair) return
  v.clock.startTime = pair.start.clone()
  v.clock.stopTime = pair.stop.clone()
  v.clock.currentTime = pair.start.clone()
  v.clock.multiplier = pair.speed
  v.clock.shouldAnimate = true
  v.trackedEntity = pair.iconEntity
}
...
function generateArcPath(from: any, to: any, height: number, factor: number): { lng: number; lat: number; alt: number }[] {
  ...
  const p0 = Cesium.Cartesian3.fromDegrees(from.lng, from.lat, from.alt)
  const p2 = Cesium.Cartesian3.fromDegrees(to.lng, to.lat, to.alt)
  const points: { lng: number; lat: number; alt: number }[] = []
  for (let i = 0; i <= 100; i++) {
    const t = i / 100
    const cart = new Cesium.Cartesian3()
    Cesium.Cartesian3.lerp(p0, controlPoint, t, cart)
    Cesium.Cartesian3.lerp(cart, p2, t, cart)
...
function updateFlightTracks(flights: FlightTrackConfig[]) {
  const v = getViewer()
  flightTrackEntities.forEach(({ pathEntity, iconEntity }, id) => {
    if (!flights.find(f => f.id === id)) { v.entities.remove(pathEntity); v.entities.remove(iconEntity); flightTrackEntities.delete(id) }
  })
  flights.forEach(flight => {
    ...
    const pathProp = new Cesium.SampledPositionProperty()
    const iconProp = new Cesium.SampledPositionProperty()
    const start = v.clock.currentTime || Cesium.JulianDate.now()
    const interval = 1
    const totalSeconds = arcPoints.length * interval
    ...
    const useModel = !!flight.modelUrl
    const pathEntity = v.entities.add({
      position: pathProp,
    ...
    const iconEntity = v.entities.add({
      position: iconProp,
      orientation: useModel ? new Cesium.VelocityOrientationProperty(iconProp) : undefined,
      billboard: useModel ? undefined : {
        image: defaultPlaneIcon,
        width: 80,
        height: 40,
        verticalOrigin: Cesium.VerticalOrigin.CENTER,
        rotation: new Cesium.CallbackProperty((time: Cesium.JulianDate) => {
...
function startFlightTrack(id: string) {
  const v = getViewer()
  const pair = flightTrackEntities.get(id)
  if (!pair) return
  v.clock.startTime = pair.start.clone()
  v.clock.stopTime = pair.stop.clone()
  v.clock.currentTime = pair.start.clone()
...
function openPopup(cartesian: Cesium.Cartesian3, data: any) {
  const v = getViewer()
  const pos = Cesium.SceneTransforms.worldToWindowCoordinates(v.scene, cartesian)
  if (!pos) return
  popupPosition.value = { x: pos.x, y: pos.y - 40 }
...
onMounted(async () => {
  ...
  v.dataSources.add(lineDataSource)
...
onUnmounted(() => { viewer.value?.destroy(); viewerReady.value = false })
...
function clearAllLayers() {
  ...
  lineDataSource.entities.removeAll()
  polyDataSource.entities.removeAll()
  dataSourceCluster.entities.removeAll()
  vehicleTrackEntities.forEach(({ pathEntity, iconEntity }) => { v.entities.remove(pathEntity); v.entities.remove(iconEntity) })
  vehicleTrackEntities.clear()
  flightTrackEntities.forEach(({ pathEntity, iconEntity }) => { v.entities.remove(pathEntity); v.entities.remove(iconEntity) })
  flightTrackEntities.clear()
  if (heatmapEntity) { v.entities.remove(heatmapEntity); heatmapEntity = null }
...
.cesium-popup { position: absolute; z-index: 1000; background: white; border: 1px solid #ccc; padding: 8px; border-radius: 4px; transform: translate(-50%, -100%); pointer-events: auto; }
</style>
