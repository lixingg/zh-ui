...
export class SignatureElement {
	...
	private nodeData: XmlData | any// 最外层的svg的数据
	private ofdPage: XmlData // 当前签名所对应的页面数据
	private ofdDocument: OfdDocument // 能拿到图片资源的公共资源数据节点
	private viewContainer: HTMLDivElement | SVGSVGElement | any // svg的包裹的，每个组件都有一个svg包裹，比如path带有一个，而text则是svg包裹text，然后text包裹tspan这样子
	...
	private mediaNodeList: XmlData[] |any // 多媒体节点的数组
	...
	#addBoundary(node: XmlData) {
		// 每个对应的是一个数组
		let stampAnnotList:any = parser.findValueByTagName(node, OFD_KEY.StampAnnot)
	...
	async #addImageSvgAsync(nodeData: XmlData){
		...
		let imgData:any = nodeData.sealData
	 ...
	 * @private
	 */
	#loadImageSourceData(imgNode: XmlData): PromiseCapability<string> | any {
