...
const extractTextToCharArray = (textStr: string, deltaXList: any[], deltaYList: any[], originX: any, originY: any) => {
	...
	for (let i = 0; i < textStr.length; i++) {
		...
		let textCodePoint:any = { 'x': convertToDpi(x), 'y': convertToDpi(y), 'text': text, deltaX: tempDeltaX, deltaY: tempDeltaY  }
...
export const createTextSpan = (nodeData: XmlData, textCodeData: XmlData, textNode: Element) => {
	// 根据scale计算tspan的位置
	let hScale:any = parser.findAttributeValueByKey(nodeData, AttributeKey.HScale)
	let vScale:any = parser.findAttributeValueByKey(nodeData, AttributeKey.VScale)
	...
	let textCode:any = parser.findValueByTagName(textCodeData, OFD_KEY.TextCode)
	let textStr:any = textCode.value.toString()
...
export const getFontSize = (textObj: XmlData) => {
	let originSize:any = parser.findAttributeValueByKey(textObj, AttributeKey.FontSize)
	if (originSize) {
