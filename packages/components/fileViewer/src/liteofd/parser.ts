...
const parseSignatureData = async (ofdDocument: OfdDocument, signList: XmlData[], pageID, pageData: XmlData) => {
	...
	if (signList && signList.length > 0) {
		for (let j = 0; j < signList.length; j++) {
			...
			let tempSign:any = signList[j]
			// 查找signature.xml中的stampannot的列表
			let tempStampAnnotObj:any = findValueByTagName(tempSign, OFD_KEY.StampAnnot)
			let signPathObj:any = findValueByTagName(tempSign, OFD_KEY.SignedValue)
...
// 查找ofd节点的属性值
export const findAttributeValueByKey = (xmlData:XmlData, key: string): any => {
	let findKey = key
...
export const findFirstAttributeValueByKey = (xmlData:XmlData, key: string): any => {
	let findKey = key
	if (!findKey.startsWith("@_")) {
		findKey = `@_${key}`
	}
