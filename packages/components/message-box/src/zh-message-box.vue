<script lang="ts">
  ...
  import BlButton from '../../button/src/zh-button.vue'
    ...
    <transition name="message-box">
      <div v-show="visible_" :class="[ns.b()]">
        ...
        <div class="flex justify-end pt-0.5">
          <zh-button v-if="showCancelButton" @click="confirmAll('cancel')">{{
            cancelText
          }}</zh-button>
          <zh-button type="primary" @click="confirmAll('confirm')">{{ confirmText }}</zh-button>
        </div>
      </div>
    </transition>
  </util-modal>
