...
<template>
  <div :class="[ns.b()]">
    <div
      class="zh-input__wrapper"
      :class="[ns.is(disabled, 'disabled')]"
      @mouseenter="enter"
      @mouseleave="leave"
    >
      <span v-if="type === 'text'" class="zh-right__suffix mr-1">
        <slot name="prefix" />
      </span>
      <input
        ...
        @change="$emit('change')"
      />
      <span v-if="clearable && mouse_is" class="zh-right__suffix">
        <zh-icon
          name="close"
      ...
      <span v-if="showPassword" class="zh-right__suffix">
        <zh-icon
      ...
      <span v-if="type === 'text'" class="zh-right__suffix">
        <slot name="suffix" />
      </span>
    </div>
  </div>
