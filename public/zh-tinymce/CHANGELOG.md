...
- Removed the `tinymce1.utils.Promise` API. #TINY-8241
...
- The `fire()` function of `tinymce.Editor`, `tinymce.dom.EventUtils`, `tinymce.dom.DOMUtils`, `tinymce1.util.Observable` and `tinymce1.util.EventDispatcher` has been deprecated and will be removed in the next major release. Use the `dispatch()` function instead. #TINY-8102
...
- Added new beforeInitialize event to tinymce1.util.XHR lets you modify XHR properties before open. Patch contributed by Brent Clintel.
...
- Added new promise feature fill as tinymce1.util.Promise.
...
### Added
- Added event binding logic to tinymce1.util.XHR making it possible to override headers and settings before any request is made.
...
- Added new tinymce1.util.Color class to handle color parsing and converting.
